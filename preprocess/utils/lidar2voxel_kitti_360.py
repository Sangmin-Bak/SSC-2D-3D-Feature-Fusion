...
import time
import sys
sys.path.append("/home/sangmin/workspace/3DSemanticSceneCompletion/VoxFormer/preprocess")
import mapping
...
if __name__ == '__main__':
  ...
  if FLAGS.sequence_length==1:
    ...
    # poses = parse_poses(os.path.join('../kitti-360/data_2d_raw',  num_seq, "poses.txt"), calibration)
    poses = parse_poses(os.path.join('./kitti-360/data_poses',  num_seq, "poses.txt"), calibration)

    progress = 10

    print("Processing {} ".format(folder), end="", flush=True)
    
    scan_files = scan_files[10511:]
