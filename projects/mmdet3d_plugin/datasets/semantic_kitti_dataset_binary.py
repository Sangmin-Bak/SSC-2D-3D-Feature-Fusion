...
class SemanticKittiDataset_Binary(Dataset):
    def __init__(
        self,
        split,
        test_mode,
    ...
    def load_scans(self):
        ...
        for sequence in self.sequences:
            ...
            pseudo_glob_path = os.path.join(
                self.data_root, "dataset", "sequences_" + self.depthmodel + "_sweep"+ self.nsweep, sequence, "voxels", "*.pseudo"
            )
            # pseudo_glob_path = os.path.join(
            #     self.data_root, "sequences_msnet3d_sweep1", sequence, "voxels", "*.pseudo"
            # )
            
            glob_path = sorted(glob.glob(glob_path))
            pseudo_glob_path = sorted(glob.glob(pseudo_glob_path))
            # pseudo_glob_path = [path for path in pseudo_glob_path if int(path.split('/')[-1].split('.')[0]) % 5 == 0]
            
            for proposal_path, pseudo_path in zip(glob_path, pseudo_glob_path):
    ...
    def union2one(self, queue):
        ...
        depthes_list = [each['depth'] for each in queue]
        metas_map = {}
        ...
        queue[-1]['depth'] = DC(torch.stack(depthes_list),
                              cpu_only=False, stack=True)
    ...
    def get_data_info(self, index):
        ...
        pseudo_path = scan["pseudo_path"]

        sequence = scan["sequence"]
        filename = os.path.basename(proposal_path)
        frame_id = os.path.splitext(filename)[0]

        meta_dict = self.get_meta_info(scan, sequence, frame_id, proposal_path, pseudo_path)
    ...
    def get_meta_info(self, scan, sequence, frame_id, proposal_path, pseudo_path):
        ...
        pseudo_bin = self.read_occupancy_SemKITTI(pseudo_path)
    ...
    def get_gt_info(self, sequence, frame_id):
        ...
        if self.split == "train" or self.split == "val":
            ...
            target_1_path = os.path.join(self.label_root, sequence, frame_id + "_1_2.npy")
            # target_1_path = os.path.join(self.data_root, "dataset", "sequences", sequence, "voxels", frame_id + ".bin")
            target = np.load(target_1_path).astype(np.float32)
