...
class SemanticKittiDatasetPoint(Dataset):
    def __init__(
        self,
        split,
        test_mode,
    ...
    def get_data_info(self, index):
        ...
        if self.split == "train" or self.split == "val":
            # load ground truth
            # target_path = os.path.join(self.label_root, sequence, frame_id + "_1_1.npy")
            target_path = os.path.join(self.data_root, "dataset", "sequences", sequence, "voxels", frame_id + ".bin")
            target = self.read_occupancy_SemKITTI(target_path)
