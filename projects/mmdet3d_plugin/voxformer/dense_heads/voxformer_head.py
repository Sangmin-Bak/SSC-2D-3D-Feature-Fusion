...
class VoxFormerHead(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # self.ssc_embed = nn.Embedding((self.bev_h) * (self.bev_w) * (self.bev_z), self.embed_dims)
    ...
    def forward(self, mlvl_feats, img_metas, target):
        ...
        # ssc_queries = self.ssc_embed.weight.to(dtype) #[128*128*16, dim]
        ...
        unmasked_idx = np.asarray(np.where(proposal.reshape(-1)>0)).astype(np.int32)[0]
        masked_idx = np.asarray(np.where(proposal.reshape(-1)==0)).astype(np.int32)[0]
        ...
        # vox_feats_flatten[vox_coords[masked_idx, 3], :] = bev_queries[vox_coords[masked_idx, 3], :]
        # vox_feats_flatten = vox_feats_flatten.reshape(bs, -1, self.embed_dims)
    ...
    def save_pred(self, img_metas, y_pred):
        ...
        if self.save_dir is None:
            pred_folder = os.path.join("./voxformer", "sequences", img_metas[0]['sequence_id']) 
        else:
            pred_folder = os.path.join(self.save_dir, img_metas[0]['sequence_id'])
