...
class LMSCNet_SS(MVXTwoStageDetector):
    ...
    def foward_training(self,
        ...
        depth =  torch.from_numpy(img_metas[0]["pseudo"]).reshape(256, 256, 32).unsqueeze(0) # [1, 256, 256, 32]
        ...
        loss_occ = BCE_ssc_loss(out_level_1, target, class_weights_level_1, self.alpha)
        loss_sem_scal = sem_scal_loss(out_level_1, target)
        loss_geo_scal = geo_scal_loss(out_level_1, target)
        # losses_pts['loss_sc_level_1'] = loss_sc_level_1
        losses_pts['loss_occ'] = loss_occ
        losses_pts['loss_sem_scal'] = loss_sem_scal
        losses_pts['loss_geo_scal'] = loss_geo_scal
    ...
    def foward_test(self,
        ...
        # msnet3d
        if not os.path.exists(os.path.join("./kitti/dataset/pseudo_to_lidar", img_metas[0]['sequence_id'], 'voxels')):
            os.makedirs(os.path.join("./kitti/dataset/pseudo_to_lidar", img_metas[0]['sequence_id'], 'voxels'))
        # save_query_path = os.path.join("./kitti/dataset/sequences_msnet3d_sweep10", img_metas[0]['sequence_id'], 'voxels', frame_id + ".query_iou5203_pre7712_rec6153")
        save_query_path = os.path.join("./kitti/dataset/pseudo_to_lidar", img_metas[0]['sequence_id'], 'voxels', frame_id + ".bin")
