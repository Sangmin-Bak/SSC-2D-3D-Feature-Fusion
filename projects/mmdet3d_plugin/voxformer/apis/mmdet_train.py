...
def custom_train_stat(model,
    ...
    data = next(iter(data_loaders[0]))
    img = data['img'].data[0]
    img_metas = data['img_metas'].data[0]
    target = data['target'].data[0].cuda()

    len_queue = img.size(1)
    img = img[:, -1, ...].cuda()
    img_metas = [each[len_queue-1] for each in img_metas]
    img_feats = model.module.extract_img_feat(img, img_metas, None)
    flops, params = profile(model.module.pts_bbox_head, inputs=(img_feats, img_metas, target))
